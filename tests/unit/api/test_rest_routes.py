...
def test_post_analyze_rejects_missing_required_fields() -> None:
    app = _app_with_state(_OkPipeline())
    b64 = _valid_jpeg_b64()
    with TestClient(app) as client:
        res = client.post(
...
def test_post_analyze_rejects_unknown_fields() -> None:
    app = _app_with_state(_OkPipeline())
    b64 = _valid_jpeg_b64()
    with TestClient(app) as client:
        res = client.post(
    ...
    assert res.status_code == 422
    assert "unexpected request fields" in res.json()["error"]


def test_post_analyze_rejects_invalid_capture_ts_ms() -> None:
    app = _app_with_state(_OkPipeline())
    b64 = _valid_jpeg_b64()
    with TestClient(app) as client:
        res = client.post(
    ...
    assert res.status_code == 422
    assert "capture_ts_ms" in res.json()["error"]
