...
class _FakeClient:
    ...
    def get(self, url: str) -> _FakeResponse:
        ...
        env_model = os.getenv("AURA_VLM_MODEL_ID")
        model_ids = [
            "Qwen/Qwen2.5-VL-7B-Instruct",
            "Qwen/Qwen2.5-VL-3B-Instruct-AWQ",
            "llava-hf/llava-1.5-7b-hf",
        ]
        if env_model and env_model not in model_ids:
            model_ids.append(env_model)
...
class _FlakyParseClient(_FakeClient):
    def __init__(self, timeout: float) -> None:
            ...
            return _FakeResponse(
                {
                    "choices": [
                        {
                            "message": {
                                "content": '{"overlays":[{"bbox":{"x":0.2,"y":0.2,"width":0.3,"height":0.3}},{"bbox":{"x":0.1'
                            }
                        }
                    ]
                }
...
def test_parse_recovery_handles_concatenated_json() -> None:
    raw = '{"overlays":[{"bbox":{"x":0.1,"y":0.2,"width":0.3,"height":0.4}}]}{"junk":true}'
    from server.core.inference.vlm.qwen_vl import _extract_json_object
...
def test_benchmark_retry_path_recovers_after_parse_failure(monkeypatch: Any) -> None:
    monkeypatch.setenv("AURA_VLM_BENCHMARK_MODE", "1")
    monkeypatch.setenv("AURA_VLM_BENCHMARK_RETRY", "1")
    monkeypatch.setattr("server.core.inference.vlm.qwen_vl.httpx.Client", _FlakyParseClient)

    backend = QwenVLBackend(endpoint="http://127.0.0.1:8000/v1")
    backend.load()
    result = backend.analyze(b"fake-image-bytes", "Locate object")
...
def test_live_vllm_accuracy_against_ground_truth_when_enabled() -> None:
    ...
    pass_rate = passes / len(cases)
    print(
        f"live_benchmark_counts: pass={passes} parse_fail={parse_fail_count} "
        f"empty_overlay={empty_overlay_count} iou_fail={iou_fail_count} total={len(cases)}"
    )
    if pass_rate < threshold:
        if os.getenv("AURA_VLM_ENFORCE_ACCURACY") == "1":
            assert pass_rate >= threshold, f"pass_rate={pass_rate:.3f} < threshold={threshold:.3f}"
        pytest.xfail(f"Live pass-rate below threshold (pass_rate={pass_rate:.3f}, threshold={threshold:.3f})")
